...
func NewVisitorService(
	...
	svc := &visitorService{
		...
		settingSvc: settingSvc, memberRepo: memberRepo, residentRepo: residentRepo, flatRepo: flatRepo, txManager: txManager,
	}
	return svc, svc
}
...
func (s *visitorService) ensureApprovalActor(ctx context.Context, societyID int64, flatID int64, actorUserID int64) error {
	...
	if isOwnerOrAdmin, err := s.isOwnerOrAdmin(ctx, societyID, actorUserID); err != nil || isOwnerOrAdmin {
		return err
	}
	status := string(models.FlatResidentStatusActive)
	isPrimary := true
	resident, err := s.residentRepo.Get(ctx, &models.FlatResidentFilter{SocietyID: &societyID, FlatID: &flatID, UserID: &actorUserID, Status: &status, IsPrimary: &isPrimary})
	if err != nil {
		return err
	}
	if resident == nil {
...
func (s *visitorService) isOwnerOrAdmin(ctx context.Context, societyID int64, actorUserID int64) (bool, error) {
	active := string(models.SocietyMemberStatusActive)
	member, err := s.memberRepo.Get(ctx, models.GetSocietyMemberFilter{SocietyID: &societyID, UserID: &actorUserID, Status: &active})
	if err != nil {
		return false, err
